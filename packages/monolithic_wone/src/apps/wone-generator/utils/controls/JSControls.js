...
export default class JSControls extends EventEmitter {
  constructor(app) {
    if (instance) {
    ...
    if (typeof window !== "undefined") {
      window.initWonePattern = (woneParams) => {
        window.localStorage.setItem("woneParams", JSON.stringify(woneParams));
        this.init();
      };

      window.setWonePatternSize = (width, height) => {
        this.app.renderer.instance.setSize(width, height);
        this.app.sizes.scaleCanvas();
      };
    }
  }
